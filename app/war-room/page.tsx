...
import { PreCallView } from '@/components/war-room/PreCallView';
...
function WarRoomMeetingOrchestrator() {
  ...
  const incidentId = incidentParam
    ? (incidentParam.startsWith('#') ? incidentParam : `#${incidentParam}`)
    : '#INC-8921';

  const severity = severityParam || 'SEV-1';

  // Preload heavy modules on mount so they're already cached when the user
  // enters the war room — eliminates dynamic-import delay.
  useEffect(() => {
        ...
        <div
          className={`z-10 flex min-h-0 flex-1 flex-col ${
            showConversation
              ...
              : 'w-full max-w-none items-center justify-center px-0 text-center'
          }`}
        >
          {!showConversation ? (
            <PreCallView
