'use client';

import React, { useState } from 'react';
import { Bot, Loader2, Users, ArrowRight } from 'lucide-react';

interface PreCallWarRoomCardProps {
  incidentId?: string;
  severity?: string;
  responderCount?: number;
  isLoading?: boolean;
  onEnterWarRoom?: () => void;
  onOtherWaysToJoin?: () => void;
}

export function PreCallWarRoomCard({
  incidentId = '#INC-8921',
  severity = 'SEV-1',
  responderCount = 5,
  isLoading = false,
  onEnterWarRoom,
  onOtherWaysToJoin,
}: PreCallWarRoomCardProps) {
  const [deploySentinel, setDeploySentinel] = useState(true);
  const [userName, setUserName] = useState(
    typeof window !== 'undefined'
      ? sessionStorage.getItem('echosphere_user_name') || ''
      : ''
  );
  const [error, setError] = useState('');

  const handleJoin = () => {
    const trimmed = userName.trim();
    if (!trimmed) {
      setError('Please enter your name to join the War Room.');
      return;
    }
    sessionStorage.setItem('echosphere_user_name', trimmed);
    onEnterWarRoom?.();
  };

  return (
    <div className="flex w-full flex-col font-sans max-w-md">
      {/* 1. Header & Incident Badges */}
      <div className="mb-6">
        <div className="inline-flex items-center gap-2 rounded-md bg-slate-800/90 px-2.5 py-1 text-xs font-mono font-medium text-slate-300 border border-slate-700 mb-3">
          <span>{incidentId}</span>
          <span className="text-slate-500">•</span>
          <span className="text-rose-400 font-semibold">[{severity}]</span>
        </div>

        <h1 className="text-3xl font-semibold tracking-tight text-slate-100">
          Ready to join?
        </h1>

        {/* Responder Count with clean active green indicator dot right below heading */}
        <div className="mt-2 flex items-center gap-2 text-xs text-slate-400">
          <span className="flex h-2 w-2 rounded-full bg-emerald-500" />
          <span className="font-medium text-slate-300">{responderCount} active responders</span>
          <span>currently connected in the War Room</span>
        </div>
      </div>

      {/* Responder Name Input */}
      <div className="mb-4">
        <label className="block text-xs font-medium text-slate-300 mb-1.5 uppercase tracking-wider">
          Responder Name
        </label>
        <input
          type="text"
          placeholder="e.g., Sarah (Lead SRE)"
          value={userName}
          onChange={(e) => {
            setUserName(e.target.value);
            if (error) setError('');
          }}
          onKeyDown={(e) => {
            if (e.key === 'Enter') handleJoin();
          }}
          className="on-dark w-full px-3 py-2 rounded-lg bg-slate-950 border border-slate-800 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-slate-600 font-sans"
        />
        {error && <p className="text-xs text-rose-400 mt-1">{error}</p>}
      </div>

      {/* 2. Deploy Ambient AI Sentinel Feature Card - Clean Dark Slate Theme */}
      <div
        onClick={() => setDeploySentinel(!deploySentinel)}
        className={`mb-6 cursor-pointer rounded-xl border p-4 transition-all ${
          deploySentinel
            ? 'border-slate-700 bg-slate-900/90 shadow-sm'
            : 'border-slate-800 bg-slate-950/60 opacity-60'
        }`}
      >
        <div className="flex items-start gap-3.5">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-slate-800 border border-slate-700 text-slate-300">
            <Bot className="h-4 w-4 text-slate-300" />
          </div>
          <div className="flex-1">
            <div className="flex items-center justify-between mb-1">
              <span className="text-sm font-medium text-slate-200">
                Deploy Ambient AI Sentinel
              </span>
              <span
                className={`h-2 w-2 rounded-full ${
                  deploySentinel ? 'bg-emerald-400' : 'border border-slate-600'
                }`}
              />
            </div>
            <p className="text-xs leading-relaxed text-slate-400">
              EchoSphere audio is 100% muted. It silently parses WebRTC audio streams for contradictions and stages hotfixes via console & state ledger.
            </p>
          </div>
        </div>
      </div>

      {/* 3. Action Buttons - Professional Enterprise Zinc Theme (Zero Bright Consumer Blue) */}
      <div className="flex items-center gap-3">
        {/* Primary Join Button */}
        <button
          onClick={handleJoin}
          disabled={isLoading}
          className="flex items-center justify-center gap-2 rounded-xl bg-slate-100 px-6 py-2.5 text-xs font-semibold text-slate-950 shadow-sm transition-all hover:bg-white active:scale-95 disabled:opacity-50 cursor-pointer"
        >
          {isLoading ? (
            <>
              <Loader2 className="h-3.5 w-3.5 animate-spin text-slate-900" />
              Connecting to Agora...
            </>
          ) : (
            <>
              Join War Room
              <ArrowRight className="h-3.5 w-3.5 text-slate-900" />
            </>
          )}
        </button>

        {/* Secondary Button */}
        <button
          onClick={onOtherWaysToJoin}
          className="flex items-center gap-1.5 rounded-xl border border-slate-700/80 bg-slate-800/80 px-4 py-2.5 text-xs font-medium text-slate-300 transition-colors hover:bg-slate-700 hover:text-white cursor-pointer"
        >
          <Users className="h-3.5 w-3.5 text-slate-400" />
          <span>Other ways to join</span>
        </button>
      </div>
    </div>
  );
}

export default PreCallWarRoomCard;
