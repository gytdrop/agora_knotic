  ...
  return (
    <div className="flex flex-col h-full bg-[#18191d] text-zinc-200 font-sans text-xs select-none">
      ...
      <div className="flex items-center justify-between p-3.5 border-b border-zinc-800/80">
        ...
        <span className="text-[11px] text-zinc-400 font-mono">{events.length} milestones</span>
      </div>
        ...
        {events.map((evt, idx) => {
          ...
          return (
            <div key={evt.id} className="relative flex items-start gap-3">
              ...
              {!isLast && (
                <div className="absolute left-3.5 top-6 bottom-0 w-[1px] bg-zinc-800" />
              )}

              {/* Node Icon */}
              <div className="relative z-10 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-zinc-900 border border-zinc-700/80 shadow-sm">
              ...
              <div className="flex-1 min-w-0 pt-0.5">
                <div className="flex items-baseline justify-between gap-2">
                  <span className="font-semibold text-zinc-100 text-xs truncate">{evt.title}</span>
                  <span className="text-[10px] text-zinc-400 font-mono shrink-0">{evt.timestamp}</span>
                </div>

                {evt.description && (
                  <p className="text-zinc-300 text-[11px] mt-1 leading-relaxed">{evt.description}</p>
                )}
