...
export function IncidentHeader({
  ...
  return (
    <header className="flex flex-col w-full border-b border-slate-800/80 bg-[#16171a] px-5 py-2.5 text-slate-200 font-sans select-none transition-colors">
      ...
      <div className="flex items-center justify-between pb-1 text-xs text-slate-400">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 font-semibold text-slate-100">
            <div className="flex h-5 w-5 items-center justify-center rounded-md bg-slate-900/90 text-white font-bold text-[11px] shadow-sm">
              E
            </div>
          ...
          <ChevronRight className="h-3 w-3 text-slate-600" />
          <span className="font-mono text-slate-300 font-medium">{cleanIncidentId}</span>
          <ChevronRight className="h-3 w-3 text-slate-600" />
          <span className="text-slate-100 font-medium">War Room</span>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5 rounded-full bg-slate-900/90 px-2.5 py-0.5 border border-slate-800 text-[11px] text-slate-300">
            <span
              className={`h-1.5 w-1.5 rounded-full ${
                isConnected ? 'bg-emerald-400 shadow-[0_0_6px_rgba(52,211,153,0.6)]' : 'bg-rose-400'
              }`}
            />
            <span className="text-slate-400">{isConnected ? 'Agora RTC Connected' : 'Disconnected'}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between pt-1">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <h1 className="text-sm font-semibold text-slate-100 truncate tracking-tight" title={displayTitle}>
            {displayTitle}
          </h1>
          ...
          {/* MTTR Timer */}
          <div className="flex items-center gap-1.5 text-xs text-slate-300 font-mono shrink-0 pl-1">
            <Clock className="h-3.5 w-3.5 text-slate-400" />
            <span>{formatTimer(secondsElapsed)}</span>
          </div>

          {/* Participant count */}
          <div className="flex items-center gap-1.5 text-xs text-slate-400 shrink-0 pl-1">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {onInjectDemoTurn && (
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg border border-slate-700/40 bg-slate-950/70 hover:bg-slate-900 px-2.5 py-1 text-xs font-semibold text-slate-200 transition-colors shadow-sm cursor-pointer"
          ...
          <Link
            ...
            className="flex items-center gap-1.5 rounded-lg border border-slate-700/80 bg-slate-800/80 px-2.5 py-1 text-xs font-medium text-slate-200 hover:bg-slate-700 hover:text-white transition-colors shadow-sm"
          >
            <span>Incident</span>
          ...
          <button
            type="button"
            className="flex h-7 w-7 items-center justify-center rounded-lg border border-slate-800 bg-slate-900 text-slate-400 hover:bg-slate-800 hover:text-slate-200 transition-colors"
