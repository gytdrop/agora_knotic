...
export function WorkflowHistoryModal({
  ...
  return (
    <>
      <div className="fixed inset-0 z-45 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs font-sans">
        <div className="relative w-full max-w-3xl max-h-[85vh] flex flex-col bg-white rounded-xl shadow-2xl border border-slate-200 overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-slate-50/50">
            <div className="flex items-center gap-3">
              <div className="h-9 w-9 rounded-lg bg-slate-100 text-slate-800 flex items-center justify-center border border-slate-200">
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {loading ? (
              ...
              <div className="py-12 text-center text-slate-400 space-y-2">
                ...
                <p className="text-[11px] text-slate-400">Click &quot;Run Now&quot; to execute this pipeline.</p>
              </div>
            ) : (
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        <div
                          className={cn(
                            ...
                            exec.status === 'running' && 'bg-slate-100 text-slate-900'
                          )}
                        >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={cn(
                                ...
                                exec.status === 'running' && 'bg-slate-100 text-slate-800'
                              )}
                            >
                              {exec.status}
                            </span>
          ...
          <div className="flex items-center justify-between px-6 py-3 border-t border-slate-200 bg-slate-50 text-xs text-slate-500">
            ...
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-1.5 rounded-lg border border-slate-200 bg-white hover:bg-slate-100 font-semibold text-slate-700 cursor-pointer"
            >
              Close
            </button>
          </div>
