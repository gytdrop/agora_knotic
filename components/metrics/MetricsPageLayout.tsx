...
> = {
  ...
  SEV3: {
    ...
    short: 'Sev 3',
    color: '#3B82F6',
    bg: 'bg-slate-100 dark:bg-slate-950/60',
...
export function MetricsPageLayout() {
  ...
  const lifecycleStages = [
    ...
    { stage: 'Time to Mitigate (TTM)', pct: 55, time: '25m 10s', color: 'bg-slate-900', desc: 'Canary rollback' },
    { stage: 'Full Resolution (TTR)', pct: 30, time: '13m 04s', color: 'bg-slate-900', desc: 'Verification complete' },
  ];
    ...
    <div className="flex min-h-screen flex-col md:flex-row bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-50 font-sans antialiased">
      ...
      <div className="flex flex-1 flex-col min-w-0 min-h-screen">
        ...
        <main className="flex-1 max-w-7xl w-full mx-auto px-4 py-6 sm:px-6 sm:py-8 space-y-6">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
            <div>
              ...
              <div className="flex items-center gap-2 text-xs font-medium text-slate-400 mb-1">
                ...
                <span>/</span>
                <span className="text-slate-700 dark:text-slate-300 font-semibold">{currentDashboard}</span>
              </div>
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  <button
                    ...
                    className="flex items-center gap-2 text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100 hover:text-slate-900 dark:hover:text-slate-400 transition-colors cursor-pointer group"
                  >
                    <span>{currentDashboard}</span>
                    <ChevronDown className="h-5 w-5 text-slate-400 group-hover:text-slate-900 transition-transform" />
                  </button>

                  {/* Dashboard Switcher Dropdown Menu */}
                  {isDashboardDropdownOpen && (
                    <div className="absolute left-0 mt-2 w-64 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xl z-30 py-2 divide-y divide-slate-100 dark:divide-slate-800 text-xs">
                      ...
                      <div className="py-1">
                        {[
                          ...
                          <button
                            ...
                            className={cn(
                              'w-full text-left px-3 py-2 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/60 cursor-pointer',
                              currentDashboard === dash
                                ? 'font-bold text-slate-900 dark:text-slate-400 bg-slate-100/50 dark:bg-slate-950/20'
                                : 'text-slate-700 dark:text-slate-300'
                            )}
                          >
                            <span>{dash}</span>
                            {currentDashboard === dash && <CheckCircle2 className="h-3.5 w-3.5 text-slate-900" />}
                          </button>
                        ))}
                      </div>
                      <div className="p-2">
                        <button
                          ...
                          className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 font-medium text-slate-800 dark:text-slate-200 cursor-pointer"
                        >
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="relative">
                ...
                {isDateDropdownOpen && (
                  <div className="absolute right-0 mt-2 w-48 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xl z-30 py-1.5 text-xs">
                    {['Last 24 Hours', 'Last 7 Days', 'Last 14 Days', 'Last 30 Days', 'Last 90 Days', '1 Year'].map(
                      (range) => (
                        <button
                          ...
                          className={cn(
                            'w-full text-left px-3 py-1.5 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer flex items-center justify-between',
                            selectedRange === range ? 'font-bold text-slate-900' : 'text-slate-700 dark:text-slate-300'
                          )}
                        >
                          <span>{range}</span>
                          {selectedRange === range && <CheckCircle2 className="h-3 w-3 text-slate-900" />}
                        </button>
                      )
                    )}
                  </div>
              ...
              <button
                ...
                className={cn(
                  ...
                  comparePrior
                    ? 'border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950/40 text-slate-800 dark:text-slate-300'
              ...
              <div className="relative">
                ...
                {isExportOpen && (
                  <div className="absolute right-0 mt-2 w-44 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xl z-30 py-1.5 text-xs">
                    <button
                      type="button"
                      onClick={handleExportCSV}
                      className="w-full text-left px-3 py-1.5 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer flex items-center gap-2 text-slate-700 dark:text-slate-300"
                    >
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-lg bg-slate-950 dark:bg-slate-100 hover:bg-slate-800 dark:hover:bg-white text-white dark:text-slate-900 px-3 py-1.5 text-xs font-semibold shadow-2xs transition-colors cursor-pointer"
              >
                <Plus className="h-3.5 w-3.5" />
          ...
          <div className="rounded-xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-3.5 shadow-2xs space-y-3 text-xs">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex flex-wrap items-center gap-2">
                <div className="flex items-center gap-1.5 text-slate-400 font-semibold pr-2 border-r border-slate-200 dark:border-slate-800">
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          isSelected
                            ...
                            : 'border-slate-200 dark:border-slate-800 text-slate-400 bg-transparent hover:border-slate-300'
                        )}
                      >
                        {meta.short}
                      </button>
                ...
                <select
                  ...
                  className="bg-transparent border border-slate-200 dark:border-slate-700 rounded-lg px-2.5 py-1 text-slate-700 dark:text-slate-300 text-xs focus:outline-none focus:ring-1 focus:ring-slate-700"
                >
                  <option value="all" className="dark:bg-slate-900">All Services</option>
                  <option value="Payments Core Service" className="dark:bg-slate-900">Payments Core Service</option>
                  <option value="Database Infrastructure" className="dark:bg-slate-900">Database Infrastructure</option>
                  <option value="Authentication & SSO" className="dark:bg-slate-900">Authentication & SSO</option>
                  <option value="API Gateway & Ingress" className="dark:bg-slate-900">API Gateway & Ingress</option>
                  <option value="Checkout Application" className="dark:bg-slate-900">Checkout Application</option>
                  <option value="Integration Services" className="dark:bg-slate-900">Integration Services</option>
                </select>
                ...
                <select
                  ...
                  className="bg-transparent border border-slate-200 dark:border-slate-700 rounded-lg px-2.5 py-1 text-slate-700 dark:text-slate-300 text-xs focus:outline-none focus:ring-1 focus:ring-slate-700"
                >
                  <option value="all" className="dark:bg-slate-900">Environment: All</option>
                  ...
                  <option value="sandbox" className="dark:bg-slate-900">Sandbox</option>
                </select>
              </div>
              ...
              <div className="flex items-center gap-2">
                <div className="relative w-48 sm:w-64">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-transparent pl-8 pr-3 py-1 text-xs text-slate-900 dark:text-slate-100 placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-slate-700"
                  />
                  {searchQuery && (
                    <button
                      type="button"
            ...
            {kpis.map((kpi) => {
              ...
              return (
                <div
                  ...
                  className="rounded-xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-2xs space-y-2 cursor-pointer hover:border-slate-700/50 hover:shadow-md transition-all group"
                >
                  <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                    <span className="font-semibold uppercase tracking-wider text-[10px]">{kpi.title}</span>
                    <Icon className="h-4 w-4 text-slate-400 group-hover:text-slate-900 transition-colors" />
                  </div>

                  <div className="flex items-baseline gap-2">
                    <span className="text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-100 font-mono">
                      {kpi.value}
                    </span>
                    <span
                      className={cn(
                  ...
                  <div className="flex items-center justify-between text-[11px] pt-2 border-t border-slate-100 dark:border-slate-800 text-slate-400">
                    ...
                    <span className="font-medium text-slate-500 dark:text-slate-400 truncate">{kpi.subtitle}</span>
                  </div>
                </div>
              );
            })}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-8 rounded-xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-2xs space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800 pb-3">
                <div>
                  <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
                    <span>Incident Volume Over Time</span>
                    <span className="text-[10px] text-slate-400 font-normal">Stacked by Severity</span>
                  </h3>
                ...
                <div className="flex items-center gap-3 text-xs">
                  {(['SEV0', 'SEV1', 'SEV2', 'SEV3'] as SeverityLevel[]).map((s) => (
                    <div key={s} className="flex items-center gap-1.5">
                      ...
                      <span className="text-slate-600 dark:text-slate-400 text-[11px]">{s}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Stacked Daily Bars */}
              <div className="h-60 w-full flex items-end gap-1.5 sm:gap-2 pt-6 pb-2 px-1 border-b border-slate-100 dark:border-slate-800">
                {dailyBars.map((bar, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* X-Axis Label */}
                      <span className="text-[10px] text-slate-400 group-hover:text-slate-700 dark:group-hover:text-slate-200 mt-2 font-mono truncate w-full text-center">
                        {bar.date}
                      </span>
                    </div>
                  );
              ...
              <div className="flex items-center justify-between text-xs text-slate-400 pt-1">
                ...
                <span className="font-mono text-slate-500 font-semibold">{filteredIncidents.length} Total Incidents</span>
                <span>Current</span>
              </div>
            </div>

            {/* Chart 2: Severity Distribution Donut (4 Columns) */}
            <div className="lg:col-span-4 rounded-xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-2xs flex flex-col justify-between space-y-4">
              ...
              <div className="relative flex items-center justify-center py-4">
                <div className="relative h-44 w-44 rounded-full border-12 border-slate-100 dark:border-slate-800 border-t-red-500 border-r-orange-500 border-b-amber-500 border-l-blue-500 flex flex-col items-center justify-center shadow-inner">
              ...
              <div className="space-y-1.5 text-xs">
                {severityDistribution.map((item) => (
                  <div
                    ...
                    className="flex items-center justify-between p-1.5 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/60 cursor-pointer transition-colors"
                  >
                    <div className="flex items-center gap-2">
            ...
            <div className="lg:col-span-12 rounded-xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-2xs space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800 pb-3">
                <div>
                  <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs pt-1">
                {lifecycleStages.map((stg, idx) => (
                  <div key={idx} className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800/80 space-y-1">
                    <div className="flex items-center gap-1.5">
                      <span className={cn('h-2 w-2 rounded-full', stg.color)} />
                      <span className="font-semibold text-slate-900 dark:text-slate-100">{stg.stage}</span>
                    </div>
                    <div className="text-lg font-bold font-mono text-slate-800 dark:text-slate-200">{stg.time}</div>
            ...
            <div className="lg:col-span-6 rounded-xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-2xs space-y-4">
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                <div>
                  <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100">
                ...
                <span className="text-xs font-mono text-slate-400">SLA Standard</span>
              </div>
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800 text-xs">
                {mttrByService.map((srv, idx) => (
                  <div
                    ...
                    className="py-3 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/40 px-1 rounded-lg cursor-pointer transition-colors"
                  >
                    <div>
            ...
            <div className="lg:col-span-6 rounded-xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-2xs space-y-4">
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                ...
                <span className="text-xs text-slate-400">Retro Insights</span>
              </div>

              <div className="space-y-3 pt-1 text-xs">
                {[
                  { reason: 'Code & Configuration Deployment', pct: 38, count: 18, color: 'bg-slate-900' },
                  { reason: 'Downstream Service Dependency Timeout', pct: 29, count: 14, color: 'bg-slate-700' },
                  { reason: 'Infrastructure Capacity & Memory Exhaustion', pct: 18, count: 9, color: 'bg-slate-700' },
                  { reason: 'Network Ingress & DNS Latency', pct: 15, count: 6, color: 'bg-amber-500' },
                ].map((rc, idx) => (
                  <div key={idx} className="space-y-1.5">
                    <div className="flex items-center justify-between font-medium">
                      <span className="text-slate-700 dark:text-slate-300">{rc.reason}</span>
                      <span className="font-mono text-slate-500">{rc.count} incidents ({rc.pct}%)</span>
                    </div>
            ...
            {customWidgets.map((cw) => (
              <div
                key={cw.id}
                className="lg:col-span-6 rounded-xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-2xs space-y-3"
              >
                <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                  <div>
                    <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100">{cw.title}</h3>
                ...
                <div className="p-6 bg-slate-50 dark:bg-slate-800/30 rounded-lg text-center text-xs text-slate-500">
                  Custom telemetry rendered for <strong className="text-slate-800 dark:text-slate-200">{cw.title}</strong>
                </div>
              </div>
            ))}
          </div>
      ...
      {selectedDrilldown && (
        <div className="fixed inset-0 z-50 flex justify-end bg-slate-900/60 backdrop-blur-xs">
          <div className="w-full max-w-xl bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 p-6 flex flex-col justify-between shadow-2xl overflow-hidden">
            <div className="flex-1 flex flex-col overflow-hidden">
              <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-800">
                <div>
                  ...
                  <p className="text-xs text-slate-900 dark:text-slate-400 font-semibold mt-1">
              ...
              {/* Incidents List */}
              <div className="flex-1 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800 pr-1 mt-4">
                {selectedDrilldown.incidents.length === 0 ? (
                  ...
                  selectedDrilldown.incidents.map((inc) => {
                    ...
                    return (
                      <div
                        key={inc.id}
                        className="py-3.5 px-2 hover:bg-slate-50 dark:hover:bg-slate-800/40 rounded-lg transition-colors space-y-2"
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-[11px] text-slate-400">{inc.startedAt}</span>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-2 text-[11px] text-slate-500 dark:text-slate-400 pt-1">
                          <span>
                            Service: <strong className="text-slate-700 dark:text-slate-300">{inc.service}</strong>
                          </span>
                          <span>
                            Commander: <strong className="text-slate-700 dark:text-slate-300">{inc.commander}</strong>
                          </span>
                        ...
                        <div className="flex items-center justify-end gap-2 pt-1">
                          <Link
                            ...
                            className="text-[11px] font-semibold text-slate-900 dark:text-slate-400 hover:underline inline-flex items-center gap-1"
                          >
            ...
            <div className="pt-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
              <button
                type="button"
                onClick={handleExportCSV}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 cursor-pointer"
              >
              ...
              <button
                type="button"
                onClick={() => setSelectedDrilldown(null)}
                className="px-4 py-1.5 rounded-lg bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900 text-xs font-semibold hover:bg-slate-800 cursor-pointer"
              >
                Close Drawer
              </button>
            </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl max-w-md w-full p-5 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleAddWidget} className="space-y-4 text-xs">
              <div className="space-y-1.5">
                <label className="font-medium text-slate-700 dark:text-slate-300">Widget Title</label>
                <input
                  type="text"
                  ...
                  required
                  className="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-transparent p-2.5 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-slate-700"
                />
              </div>

              <div className="space-y-1.5">
                <label className="font-medium text-slate-700 dark:text-slate-300">Visualization Type</label>
                <select
                  value={newWidgetType}
                  onChange={(e) => setNewWidgetType(e.target.value as 'stat' | 'bar' | 'donut' | 'ranking')}
                  className="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-transparent p-2.5 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-slate-700"
                >
                  <option value="stat" className="dark:bg-slate-900">Single Stat KPI</option>
                  <option value="bar" className="dark:bg-slate-900">Stacked Bar Chart</option>
                  <option value="donut" className="dark:bg-slate-900">Donut Chart</option>
                  <option value="ranking" className="dark:bg-slate-900">Ranked Service Table</option>
                </select>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <label className="font-medium text-slate-700 dark:text-slate-300">Dataset</label>
                  <select
                    value={newWidgetDataset}
                    onChange={(e) => setNewWidgetDataset(e.target.value as 'volume' | 'mttr' | 'mttd' | 'mtta')}
                    className="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-transparent p-2.5 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-slate-700"
                  >
                    <option value="volume" className="dark:bg-slate-900">Incident Volume</option>
                    <option value="mttr" className="dark:bg-slate-900">MTTR</option>
                    <option value="mttd" className="dark:bg-slate-900">MTTD</option>
                    <option value="mtta" className="dark:bg-slate-900">MTTA</option>
                  </select>
                </div>

                <div className="space-y-1.5">
                  <label className="font-medium text-slate-700 dark:text-slate-300">Group By</label>
                  <select
                    value={newWidgetGroupBy}
                    onChange={(e) => setNewWidgetGroupBy(e.target.value as 'severity' | 'service' | 'environment')}
                    className="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-transparent p-2.5 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-slate-700"
                  >
                    <option value="severity" className="dark:bg-slate-900">Severity</option>
              ...
              <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                ...
                <button
                  type="submit"
                  className="px-3.5 py-1.5 rounded-lg bg-slate-900 hover:bg-slate-800 text-white font-semibold shadow-2xs cursor-pointer"
                >
                  Add to Dashboard
                </button>
              </div>
